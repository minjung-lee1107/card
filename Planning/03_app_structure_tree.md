preprocess.py
├─ import
│  ├─ pandas as pd
│  ├─ re
│  ├─ json
│  ├─ difflib
│  ├─ OpenAI
│  ├─ typing (Tuple, Dict)
│  └─ streamlit as st
│
├─ 🧹 데이터 전처리 & 자동 분류
│  │
│  ├─ 📂 카테고리 정의
│  │  ├─ RULES            (키워드 룰 기반 분류 사전)
│  │  └─ CATEGORIES       (최종 카테고리 목록)
│  │
│  ├─ 🧼 텍스트/컬럼 정규화
│  │  ├─ _norm_col        (컬럼명 비교용 정규화)
│  │  └─ normalize_text   (description 텍스트 정규화)
│  │
│  ├─ 🏷️ 카테고리 분류
│  │  ├─ rule_category            (RULES 기반 1차 분류)
│  │  ├─ ai_category_batch        (OpenAI 배치 분류)
│  │  ├─ ai_category_cached       (캐시 기반 단건 분류)
│  │  ├─ ai_category_batched      (여러 건 묶음 분류)
│  │  └─ build_category           (rule → 미분류만 AI → 최종 category)
│  │
│  ├─ 🗂️ 컬럼 자동 매핑
│  │  ├─ STANDARD_COLS    (표준 스키마 정의)
│  │  ├─ REQUIRED_COLS    (필수 컬럼 정의)
│  │  ├─ SYNONYMS         (표준 컬럼 동의어 사전)
│  │  └─ auto_map_columns (사전 매핑 + difflib 유사도 보완 + 불필요 컬럼 drop)
│  │
│  ├─ 🔄 타입 변환 / 값 정리
│  │  └─ coerce_types
│  │     ├─ date: datetime 변환 + 결측 제거
│  │     ├─ amount: 숫자 변환(콤마/원 제거) + 결측 제거
│  │     ├─ is_fixed: bool 변환
│  │     └─ installment_type/months 정규화
│  │
│  └─ 🚀 전처리 파이프라인 (엔트리 포인트)
│     └─ preprocess_any_expense_df
│        ├─ auto_map_columns
│        ├─ coerce_types
│        ├─ 필수 컬럼 검사(date/amount/description)
│        ├─ category 보정(없거나 비면 build_category로 생성)
│        ├─ category 표준화(목록 외 → 기타)
│        ├─ 파생 컬럼 생성(month, year_month 등)
│        ├─ 컬럼 정렬
│        └─ 반환: (df_processed, report)
│
app.py
├─ import
│  ├─ streamlit as st
│  ├─ pandas as pd
│  ├─ numpy as np
│  ├─ plotly.express as px
│  ├─ plotly.graph_objects as go
│  ├─ OpenAI
│  ├─ re / json / difflib
│  └─ preprocess_any_expense_df (from utils.preprocess)
│
├─ 🔑 OpenAI 설정
│  ├─ api_key = st.secrets.get("OPENAI_API_KEY")
│  └─ client = OpenAI(api_key) if 존재 시
│
├─ 🎨 Streamlit 초기 설정
│  ├─ st.set_page_config
│  ├─ st.title
│  └─ Session State 초기화
│     ├─ df_raw
│     ├─ df_processed
│     ├─ prep_report
│     ├─ file_uploaded
│     └─ file_name
│
├─ 📂 파일 업로드 영역
│  ├─ Sidebar: st.file_uploader (csv/xlsx/xls)
│  ├─ 옵션 토글
│  │  ├─ AI 자동 보정 사용
│  │  └─ 표준 컬럼 외 컬럼 삭제
│  │
│  └─ uploaded_file 처리
│     ├─ 새 파일 감지 (파일명 비교)
│     ├─ 파일 읽기
│     │  ├─ csv → utf-8 → 실패 시 cp949
│     │  └─ excel → read_excel
│     ├─ session_state 저장
│     ├─ 전처리 실행 (1회만)
│     │   ├─ preprocess_any_expense_df 호출
│     │   ├─ 실패 시 오류 안내 + 매핑 결과 expander + st.stop
│     │   └─ 성공 시 df_processed/prep_report 저장
│     └─ 이후 재실행 시 캐시 재사용
│
├─ 🎛️ Sidebar: 필터
│  ├─ 기간 필터 (st.date_input)
│  ├─ 카테고리 필터 (st.multiselect)
│  └─ 일시불/할부 필터 (installment_type 기반 multiselect)
│
├─ 📊 대시보드: 핵심 지표 (Metrics)
│  ├─ 총 지출
│  ├─ 월평균 지출
│  ├─ 최대 단일 지출
│  └─ 거래 건수
│
├─ 📈 대시보드: 차트 영역
│  ├─ 2열 레이아웃 (col_left, col_right)
│  │
│  ├─ (Left) 도넛 차트
│  │  ├─ st.segmented_control
│  │  │   └─ 카테고리 / 일시불·할부 전환
│  │  ├─ 카테고리 도넛 (px.pie)
│  │  └─ 일시불·할부 도넛
│  │     ├─ 금액/건수 선택
│  │     └─ pay_type 집계 후 px.pie
│  │
│  └─ (Right) 막대 그래프
│     └─ 카테고리별 월 누적 지출 (최근 6개월, stack)
│
├─ 📆 추이 분석
│  ├─ st.segmented_control
│  │   └─ 월별 / 주별 / 요일별 / 일별
│  │
│  ├─ 공통 함수: draw_line
│  ├─ 월별 지출 추이 (year_month)
│  ├─ 주별 지출 추이
│  ├─ 일별 지출 막대그래프
│  │   ├─ 월 선택(selectbox)
│  │   ├─ 일별 합계 + 거래건수
│  │   ├─ 주말/평일 색 구분
│  │   └─ 최고지출일 표시
│  │
│  └─ 요일별 지출 히트맵
│      ├─ ISO week_key 생성
│      ├─ (week_key x weekday) pivot
│      └─ px.imshow
│
├─ 🔀 비교 분석
│  ├─ 두 달 선택
│  ├─ category×month pivot + diff 계산
│  ├─ 슬로프/라인 차트 (px.line)
│  └─ Top N 변화 요약
│
├─ 💻 데이터 요약 통계
│  └─ generate_expense_summary
│     ├─ 기본 통계
│     ├─ 개월 수 계산
│     ├─ 월 평균 총지출
│     ├─ 카테고리별 통계
│     └─ 반환: summary(dict)
│
│
ai_and_report.py
├─ 💡 AI 인사이트
│  └─ get_ai_insights(summary_data)
│     ├─ 요약 데이터 기반 Prompt 구성
│     ├─ 월 기준 계산 강제
│     ├─ 패턴/개선부문/권장예산/이자 시뮬 포함
│     ├─ OpenAI 호출
│     │   ├─ model="gpt-4o-mini"
│     │   ├─ max_tokens=1000
│     │   └─ temperature 설정
│     └─ 결과 반환
│
├─ 📄 월간 리포트
│  └─ generate_monthly_report(df, insights=None)
│     ├─ 요약 지표 계산
│     ├─ 카테고리별 합계 + 비율
│     ├─ 고정비 집계
│     ├─ 최고 지출일
│     ├─ Top5 지출
│     ├─ AI 인사이트 포함
│     └─ Markdown 문자열 반환
│
app.py
└─ 🖥️ UI 연동
    ├─ "🔍 AI 분석 시작" 버튼
    │   ├─ summary 생성
    │   ├─ get_ai_insights 호출
    │   └─ session_state 저장
    │
    ├─ 이전 인사이트 expander
    │
    ├─ "📄 리포트 생성" 버튼
    │   ├─ generate_monthly_report 호출
    │   └─ st.markdown 출력
    │
    └─ 다운로드 버튼
        ├─ 파일명: expense_report_YYYYMMDD.md
        └─ mime="text/markdown"