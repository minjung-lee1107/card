utils/
├─ preprocess.py
│  ├─ import
│  │  ├─ pandas as pd
│  │  ├─ re / json / difflib
│  │  ├─ typing (Tuple, Dict, Optional ...)
│  │  ├─ streamlit as st
│  │  └─ OpenAI
│  │
│  ├─ 📂 카테고리 정의
│  │  ├─ RULES        (키워드 룰 기반 분류 사전)
│  │  └─ CATEGORIES   (최종 카테고리 목록)
│  │
│  ├─ 🧼 텍스트/컬럼 정규화
│  │  ├─ _norm_col()        (컬럼명 비교용 정규화)
│  │  └─ normalize_text()   (description 텍스트 정규화)
│  │
│  ├─ 🏷️ 카테고리 분류 로직
│  │  ├─ rule_category()          (RULES 기반 1차 분류)
│  │  ├─ ai_category_batch()      (OpenAI 배치 분류)
│  │  ├─ ai_category_cached()     (캐시 기반 단건 분류)
│  │  ├─ ai_category_batched()    (여러 건 묶음 분류)
│  │  └─ build_category()         (rule → 미분류만 AI → 최종 category)
│  │
│  ├─ 🗂️ 컬럼 자동 매핑
│  │  ├─ STANDARD_COLS       (표준 스키마 정의)
│  │  ├─ REQUIRED_COLS       (필수 컬럼 정의: date/amount)
│  │  ├─ SYNONYMS            (동의어 사전)
│  │  └─ auto_map_columns()  (사전 매핑 + difflib 보완 + 옵션에 따라 컬럼 정리)
│  │
│  ├─ 🔄 타입 변환 / 값 정리
│  │  └─ coerce_types()
│  │     ├─ date: datetime 변환 + 결측 제외
│  │     ├─ amount: 숫자 변환(콤마/원 제거) + 결측 제외
│  │     ├─ is_fixed: bool 변환
│  │     └─ installment_type / installment_months 정규화
│  │
│  └─ 🚀 전처리 파이프라인 (엔트리)
│     └─ preprocess_any_expense_df()
│        ├─ auto_map_columns()
│        ├─ coerce_types()
│        ├─ 필수 컬럼 검사 (date/amount)
│        ├─ category 보정
│        │  ├─ (없거나 비면) description 있으면 build_category()로 생성
│        │  └─ (없거나 비면) description 없으면 "기타"로 채움
│        ├─ category 표준화 (목록 외 → "기타")
│        ├─ 파생 컬럼 생성 (month, year_month 등)
│        ├─ 컬럼 정렬
│        └─ return: (df_processed, report)
│
├─ ai_and_report.py
│  ├─ 💡 AI 인사이트
│  │  └─ get_ai_insights(summary_data)
│  │     ├─ 요약 데이터 기반 Prompt 구성
│  │     ├─ "월 기준 계산" 강제
│  │     ├─ 패턴/개선/권장예산/이자 시뮬 포함
│  │     └─ OpenAI 호출
│  │        ├─ model="gpt-4o-mini"
│  │        ├─ max_tokens=1000
│  │        └─ temperature=...
│  │
│  └─ 📄 월간 리포트
│     └─ generate_monthly_report(df, insights=None)
│        ├─ 요약 지표 계산
│        ├─ 카테고리별 합계/비율
│        ├─ 고정비 집계
│        ├─ 최고 지출일
│        ├─ Top N 지출
│        ├─ (선택) AI 인사이트 포함
│        └─ return: Markdown 문자열
│
├─ sample_data_code.py
│  ├─ import / 상수 / 룰 정의 (CATEGORY_RULES, MERCHANT_RULES ...)
│  ├─ 유틸: random_date(), random_amount(), get_merchant()
│  ├─ make_sample_expense_data()  (샘플 DataFrame 생성)
│  ├─ save_sample_csv()           (CSV 저장)
│  └─ if __name__ == "__main__":  (로컬 실행 엔트리)

project_root/
├─ app.py
│  ├─ import
│  │  ├─ streamlit as st
│  │  ├─ pandas / numpy / random
│  │  ├─ plotly.express / plotly.graph_objects
│  │  ├─ re / json / difflib
│  │  ├─ OpenAI
│  │  ├─ preprocess_any_expense_df (from utils.preprocess)
│  │  └─ get_ai_insights, generate_monthly_report (from utils.ai_and_report)
│  │
│  ├─ 🔑 OpenAI 설정
│  │  ├─ api_key = st.secrets.get("OPENAI_API_KEY")
│  │  └─ client = OpenAI(api_key) if api_key else None
│  │
│  ├─ 🎨 Streamlit 초기 설정
│  │  ├─ st.set_page_config / st.title
│  │  └─ Session State 초기화
│  │     ├─ df_raw / df_processed / prep_report
│  │     ├─ file_uploaded / file_name
│  │     └─ last_insights (선택)
│  │
│  ├─ 📂 Sidebar: 업로드/옵션
│  │  ├─ file_uploader (csv/xlsx/xls)
│  │  ├─ 옵션 토글
│  │  │  └─ "카테고리 AI 자동 보정 사용" (업로드 전 노출)
│  │  └─ sample data 다운로드 (csv)
│  │
│  ├─ 🧾 업로드 파일 처리 & 전처리(1회)
│  │  ├─ (업로드 후) 전처리 옵션 토글
│  │  │  └─ "표준 컬럼 외 컬럼 삭제" (drop_non_standard)
│  │  ├─ 새 파일 감지 (파일명 비교)
│  │  ├─ 파일 로드
│  │  │  ├─ csv: utf-8 → 실패 시 cp949
│  │  │  └─ excel: read_excel
│  │  ├─ session_state 저장 (원본/상태)
│  │  ├─ preprocess_any_expense_df() 실행
│  │  │  ├─ 실패: 오류 안내 + 매핑결과 expander + st.stop()
│  │  │  └─ 성공: df_processed/prep_report 저장
│  │  └─ 이후 rerun: 캐시 재사용
│  │
│  ├─ 🎛️ Sidebar: 필터
│  │  ├─ 기간 (st.date_input)
│  │  ├─ 카테고리 (st.multiselect)
│  │  └─ 결제유형 (일시불/할부 multiselect)
│  │
│  ├─ 📊 대시보드
│  │  ├─ 핵심 지표 Metrics
│  │  │  ├─ 총 지출 / 월평균 / 최대 / 거래건수
│  │  └─ 차트 영역 (2열)
│  │     ├─ (Left) 도넛 차트
│  │     │  ├─ segmented_control: "카테고리" vs "일시불·할부"
│  │     │  ├─ 카테고리 도넛 (px.pie)
│  │     │  └─ 일시불·할부 도넛
│  │     │     ├─ 금액/건수 선택
│  │     │     └─ pay_type 집계 후 px.pie
│  │     └─ (Right) 막대 그래프
│  │        └─ 카테고리별 월 누적 지출 (최근 6개월, stack)
│  │
│  ├─ 📆 추이 분석
│  │  ├─ segmented_control: 월별/주별/요일별/일별
│  │  ├─ 공통 함수: draw_line()
│  │  ├─ 월별 추이 (year_month)
│  │  ├─ 주별 추이
│  │  ├─ 일별 막대
│  │  │  ├─ 월 선택
│  │  │  ├─ 일별 합계 + 건수
│  │  │  └─ 최고 지출일 표시
│  │  └─ 요일별 히트맵
│  │     ├─ ISO week_key 생성
│  │     ├─ pivot (week_key x weekday)
│  │     └─ px.imshow
│  │
│  ├─ 🔀 비교 분석
│  │  ├─ 두 달 선택
│  │  ├─ category×month pivot + diff
│  │  ├─ 변화 시각화 (px.line 등)
│  │  └─ Top N 변화 요약
│  │
│  └─ 🤖 AI 분석 & 리포트
│     ├─ "🔍 AI 분석 시작" 버튼
│     │  ├─ summary 생성
│     │  ├─ get_ai_insights() 호출
│     │  └─ session_state.last_insights 저장
│     ├─ 이전 인사이트 expander
│     ├─ "📄 리포트 생성" 버튼
│     │  ├─ generate_monthly_report() 호출
│     │  └─ st.markdown 출력
│     └─ 다운로드 버튼
│        ├─ file_name: expense_report_YYYYMMDD.md
│        └─ mime: text/markdown (또는 txt 옵션)